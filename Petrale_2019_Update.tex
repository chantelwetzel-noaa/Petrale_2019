...
dropped below the managment target (25\%) for the first time in 1976.
...
assesssment estimated the stock to have rebuilt to the management target
...
Exploitation rate & 0.091 & 0.062 & 0.074 & 0.111 & 0.126 & 0.138 & 0.129 & 0.155 & 0.152 &  \\ 
  Age 3+ biomass (mt) &  9487.05 & 12726.80 & 15602.80 & 17806.70 & 18973.00 & 19748.50 & 19715.50 & 19637.00 & 18918.70 & 18265.30 \\ 
   \hline
Spawning Output &  3494 &  4414 &  5904 &  7751 &  9284 & 10202 & 10439 & 10531 & 10213 &  9867 \\ 
...
spawning taking place from December to February (Harry
\protect\hyperlink{ref-harry_analysis_1956}{1956}, Best
...
et al. \protect\hyperlink{ref-reilly_recreational_1994}{1994}, Castillo
\protect\hyperlink{ref-castillo_latitudinal_1995}{1995}, Love
...
Forrest \protect\hyperlink{ref-alderdice_effects_1971}{1971}).
...
estimated total dead are shown in Table \ref{tab:mnmgt_perform_table}.
...
\begin{enumerate}
  ...
  \item NWFSC West Coast Groundfish Bottom Trawl Survey (2015-2018).
\end{enumerate}
...
spatio-temporal delta model implemented as an R package, VAST (Thorson
and Barnett \protect\hyperlink{ref-thorson_comparing_2017}{2017}), which
...
user manual.pdf). The stratification and modeling configuration are
...
\ref{fig:nw_cpue_depth} shows that the postive tows catch rate by depth
peaks between 100-200 meters and declines as depth increases.

ADD INFORMATION ON THE DATA USED FOR PETRALE

Figure \ref{fig:nw_qq} Figure \ref{tab:Index_Summary}
...
distributions for the NWFSC survey from 2003-2018 generally show a
strong cohort growing through 2005 and smaller fish entering the
population beginning in 2007 rows with a large 2014 cohort entering the
populations (Figure \ref{fig:nw_len_freq}).
...
the number of ages taken by the survey. These data show the growth
...
West Coast Groundfish Bottom Trawl Survey age-compositions, which allow
for easier viewing of strong cohorts, show the strong 1998 cohort ageing
from 2003 to 2007, with younger fish appearing in 2008-2014 (Figure
...
all fishery-independent surveys was set equal to the number of tows that
sampled petrale sole by year to be consistent with the 2015 update
assessment. The effective sample size of conditional-age-at-length data
was set at the number of fish at each length by sex and by year. The
conditional-age-at-length data were not expanded and were binned by
...
methods are most recently described in Weinberg et al. ({\textbf{???}}).
Its basic design was a series of equally-spaced transects from which
searches for tows in a specific depth range were initiated. The survey
design has changed slightly over the period of time. In general, all of
the surveys were conducted in the mid-summer through early fall: the
1977 survey was conducted from early July through late September; the
surveys from 1980 through 1989 ran from mid-July to late September; the
1992 survey spanned from mid-July through early October; the 1995 survey
was conducted from early June to late August; the 1998 survey ran from
early June through early August; and the 2001 and 2004 surveys were
...
used for the early and late Triennial Survey (Table
...
The input sample sizes for length data were based on the number of tows
that sampled petrale sole by year.
...
for Oregon) extacted ADD DATE XXX. Historical catches were not updated
...
landings. The non-trawl landings are included in the trawl landings but
do not include discarded petrale sole (Table \ref{tab:Discard}).
...
known. The discard rates from WCGOP are shown in Table
\ref{tab:Discard}.
...
of petrale sole by these vessels were very low, near zero, and were not
included in this update assessment.
...
Commerical logbook data for petrale sole was first used to construct
CPUE indices of abundance in the 1999 assessment for Oregon fleets from
1987-1997 (Sampson and Lee
\protect\hyperlink{ref-sampson_assessment_1999}{1999}). In West Coast
assessments since the first inclusion 1999 the commercial CPUE indices
have been extended and or updated based on management changes and new
statistical methods. For additional information on the use of CPUE
indices in the assessmnet of petrale sole please see the 2013 assessment
(Haltuch et al.
\protect\hyperlink{ref-haltuch_status_2013}{2013}\protect\hyperlink{ref-haltuch_status_2013}{b})
...
(Haltuch et al.
\protect\hyperlink{ref-haltuch_status_2013}{2013}\protect\hyperlink{ref-haltuch_status_2013}{b}).
Two CPUE indices from 1987-2009 with catchability modeled as a power
function are used in this update assessment, one for the north and south
winter fisheries.

\subsubsection{Fishery Length and Age
Data}\label{fishery-length-and-age-data}

The PacFIN BDS database contains data from ODFW (1966-present) and WDFW
(1955- present), but only 1985-present data from CDFG. The CDFG dataset
for the years 1948-1922 was extracted and provided from CALCOM by Brenda
Erwin (CDFG) in 2011.
...
the combined Wasington and Oregon fleet. Age frequencies were computed
...
are summarized by the number of tows (Table XX -Table XX). Figures
\ref{fig:north_lengths}, \ref{fig:south_lengths}, and
\ref{fig:comm_ages} show plots of the commercial length and age
composition data.

The input sample size for commercial lengths and ages were calculated
based on the number of tows by years.
...
be consistant with the full assessment this update assessment retains
the equal sex ratio assumption. However, examining the NWFSC West Coast
Groundfish Bottom Trawl Survey data there appears to be a higher
proportion of males in the population across the mid-range lengths with
the proportion rising to 1 at the largest lengths due to dimorphic
...
(Figures \ref{fig:mod_wt_length}).

\subsubsection{Growth (Length-at-Age)}\label{growth-length-at-age}

The length-at-age was estimated for male and female petrale sole. Figure
\ref{fig:Len_Age} shows the lengths and ages as well as predicted von
Bertalanffy fits to the data to the NWFSC West Coast Groundfish Bottom
Trawl Survey data. Females grow larger than males and sex-specific
...
This update assessment was unable to generate new analyses to evaluate
potential ecosystem data and methodologies for this stock assessment for
consistency with the last full assessment.
...
\item NWFSC West Coast Groundfish Bottom Trawl Survey index was calculated using VAST.

\item Added composition data from the commercial fishery (length and age data 2015-2018) and NWFSC West Coast Groundfish Bottom Trawl Survey (length and age data 2015-2018).

\item Corrected NWFSC West Coast Groundfish Bottom Trawl Survey length and age composition data expansion (2003-2018).
...
(\protect\hyperlink{ref-hamel_method_2015}{2015}). The natural mortality
prior was updated for this update assessment using the Hamel
meta-analysis approach. Maximum age was assumed to be 32 and 29 years
for females and males, respectively, the same assumption regarding
maximum age as applied in the 2013 assessment.

\subsubsection{Data Weighting}\label{data-weighting}

Length and age-at-length compositions from the NWFSC West Coast
Groundfish Bottom Trawl Survey were fit along with length and marginal
age compositions from the fishery and other survey fleets. Length data
started with a sample size determined from the equation listed in
Sections \ref{nwfsc-west-coast-groundfish-bottom-trawl-survey} (survey
data) and \ref{fishery-length-and-age-data} (fishery data). It was
assumed for age-at-length data that each age was a random sample within
the length bin and the model started with a sample size equal to the
number of fish in that length bin.

XXX - I think they may have done Harmonic Mean Weighting - check The
update assessment model was weighted using the ``Francis method'', which
was based on equation TA1.8 in Francis
...
\begin{figure}
\centering
\includegraphics{Figures/NWFSC Shelf-Slope_Length_Frequency.png}
...
\begin{figure}
\centering
\includegraphics{Figures/NWFSCBT_Age_Frequency.png}
...
\begin{figure}
\centering
\includegraphics{r4ss/plots_mod1/bio5_weightatsize.png}
\caption{Estimated weight-at-length for female and male petrale sole.
\label{fig:mod_wt_length}}
\end{figure}

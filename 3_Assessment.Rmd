...
###Response to the Most Recent STAR Panel Recommendations
The most recent STAR panel meeting for petrale sole was for the 2013 full assessment.  For clarity the petrale sole specific recommendations from the STAR panel are presented here, but given that this was an update assessment which limits model changes, these items have not been formally addressed.
...
Fixed parameters in the model were as follows. The standard deviation of the recruitment deviates was fixed at `r print(mod1$parameters[mod1$parameters$Label == "SR_sigmaR","Value"],2)`. Maturity-at-length and fecundity was fixed as described above in Section \ref{maturation-and-fecundity}. Length-weight parameters were fixed at estimates using all length-weight observations (Figure \ref{fig:wt_length}).

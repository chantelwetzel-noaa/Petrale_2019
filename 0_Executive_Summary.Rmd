...
\item `r nwfsc` index of abundance calculated using VAST.

\item Updated `r nwfsc` length and age data (2015-2018) with all years data expanded to the tow and strata.

\item `r full.tri` early and late index of abundance calculated using VAST.
...
The 2009 stock assessment estimated `r spp` to be at 11.6% of unfished spawning stock biomass in 2010. Based on the 2009 stock assessment, the 2010 coast-wide ACL was reduced to 1,200 mt to reflect the overfished status of the stock and the 2011 coast-wide overfishing limit (OFL) and ACL were set at 1,021 mt and 976 mt, respectively (Table \ref{tab:mnmgt_perform}). 

Recent coast-wide annual landings have not exceeded the ACL. The 2009, 2011, and 2013 full assessments estimated that `r spp` have been below the management target since the 1960s and below the overfished threshold between the early 1980s and the early 2000s with fishing mortality rates in excess of the current F-target for flatfish of SPR30%. The 2015 update assessment estimated that the stock had recovered with the relative spawning biomass exceeding the management target.
...
##Decision Table{-}    
The forecast of stock abundance and yield was developed using the base model. The total catches in 2019 and 2020 are set to the PFMC adopted ACLs of 2908 and 2845, respectively. The exploitation rate for 2021 and beyond is based upon an SPR of 30% and the 25:5 harveest control rule. The average exploitation rates, across recent years, by fleet as provided by the Groundfish Management Team were used to distribute catches among the fisheries during the forecast period. 

Uncertainty in the forecasts is based upon the three states of nature based on the likelihood profile of female M, chosen using a change of 1.2 NLL units (75% interval) from the minimum value to correspond to the midpoints of the lower 25% probability and upper 25% probability regions from the base model and are low (0.125, rounded to the third decimal place) and high (0.180, rounded to the third decimal place) values for female natural mortality. 

Catches during the projection period under the current harvest control rule are projected to start around 3800 mt and decline over the projection period to 3000 -3050 mt as the stock declines towards that target spawning biomass (Table \ref{tab:OFL_projection}).  Acorss the low and high states of nature the under the current harvest control rule range between 0.25 - 0.38 fraction of spawning biomass by the end of the projection period (Table \ref{tab:Decision_table_mod1_es}).

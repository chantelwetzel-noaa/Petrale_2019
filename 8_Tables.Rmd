...
Catch_AllYrs = xtable(catch, 
                      caption = c('Landings (mt) for each fleet for the modeled years.'),
                      label = 'tab:Comm_Catch',
                      digits = 0)    

# Add alignment  
align(Catch_AllYrs) = c('lccccc')
...
addtorow$command = c(" & \\multicolumn{2}{c}{Winter N.} &  \\multicolumn{2}{c}{Winter S.} & \\multicolumn{2}{c}{Triennial Early} & \\multicolumn{2}{c}{Triennial Late} & \\multicolumn{2}{c}{NWFSC} \\\\\n",
...
Fish_Length.table = xtable(Fish_Length, 
                           caption = c('Summary of the number tows, fish sampled, and the calculated input sample size for the length data from the NWFSC West Coast Groundfish Bottom Trawl Survey used in the stock assessment.'),
...
Strata.table = xtable(Strata, 
                           caption = c('Description of the strata used to create the indices for the NWFSC West Coast Groundfish Bottom Trawl Survey.'),
...
# Index of abundance summary, create table   
Fish_Length.table = xtable(Fish_Length, 
                           caption = c('Summary of the number tows, fish sampled, and the calculated input sample size for the age data from the NWFSC West Coast Groundfish Bottom Trawl Survey used in the stock assessment.'),
...
# Index of abundance summary, create table   
Fish_Length.table = xtable(Fish_Length, 
                           caption = c('Summary of the number tows, fish sampled, and the calculated input sample size for the length data from the AFSC/NWFSC West Coast Triennial Shelf Survey used in the stock assessment.'),
...
# Index of abundance summary, create table   
Strata.table = xtable(Strata, 
                      caption = c('Description of the strata used to create the indices for the AFSC/NWFSC West Coast Triennial Shelf Survey Early (1980 - 1992) survey.'),
...
# Index of abundance summary, create table   
Strata.table = xtable(Strata, 
                      caption = c('Description of the strata used to create the indices for the AFSC/NWFSC West Coast Triennial Shelf Survey Late (1995-2004) survey.'),
...
# Index of abundance summary, create table   
Fish_Length.table = xtable(Fish_Length, 
                           caption = c('Summary of the number of fishery length samples used in the stock assessment (continued on the next page).'),
                           label = 'tab:Fishery_Lengths',
                           digits = 0)    

# Add alignment 
align(Fish_Length.table) = c('lccccccccc')
...
# Index of abundance summary, create table   
Fish_Length.table = xtable(Fish_Length, 
                           caption = c('Summary of fishery age samples used in the stock assessment (continued on the next page).'),
...
```{r , results = 'asis'}
  ...
  x = 1:18
  Age.Error = cbind(mod1$age_error_mean[x,"type1"],
...
Harmonic.table = xtable(harmonic, 
                           caption = c('Data weights applied when using McAllister Ianelli Harmonic Mean data weighting.'),
                           label = 'tab:harm',
                           digits = 3)
...
colnames(Timeseries)=c('Year', 
                       ...
                       paste('Summary biomass', min_age, '(mt)'),
                       'Relative biomass', 
                       'Age-0 recruits', 
                       'Estimated total catch (mt)',
                       '1-SPR',
...
# Add alignment
#align(Timeseries.table) = c('lccccccccc')
...
colnames(jitter) = c("Status", "Jitter = 0.05", "Jitter = 0.10")
...
len.theta = round(len.parm / (1 + len.parm), 2)
age.theta = round(age.parm / (1+ age.parm),  2)
...
#                          'Dirichlet')
 ...
 colnames(Sens_model1) = rep("", ncol(Sens_model1))
  
 hlines = c(-1, 0, nrow(Sens_model1))
 addtorow = list()
 addtorow$pos = list(0,0)
 addtorow$command = c("      &      & Low M    & High M   &  Old M  &           &            & Sex Ratio &          &          \\\\\n",
                      "Label & Base & (female) & (female) &  Prior  & Fecundity &  Sex Ratio & Fecundity & Francis & Dirichlet \\\\\n" )

# Create sensitivities table
Sens_model1.table = xtable(Sens_model1, 
                           caption = c('Sensitivity runs compared to the base model.'),
...
align(Sens_model1.table) = c('llrrrrrrrrr')
...
<!-- ***********MODEL 1 DATA ANALYSIS RUNS******************************* --> 
\begin{landscape}
```{r, results = 'asis'}

# Read in/organize sensitivity file, change column names, caption
Sens_model1 = read.csv('./txt_files/bridge_values.csv')
 ...
 addtorow = list()
 addtorow$pos = list(0,0)
 addtorow$command = c("           &       & Conversion + &         &           &           &           &       &      \\\\\n",
                      "Parameters & 2015  & Priors       & + Catch & + Survey  & + Discard &  + Length & + Age & 2019 \\\\\n" )

# Create sensitivities table
Sens_model1.table = xtable(Sens_model1, 
                           caption = c('Data analysis runs where incremental changes were made to the 2015 model.'),
                           label = 'tab:bridge_table',
                           digits = 3)


align(Sens_model1.table) = c('llrrrrrrrr')
...
decision_mod1.table = xtable( decision_mod1, 
                              caption = c(paste('Decision table summary of 10-year projections beginning in ', LastYR+2,' for alternate states of nature based on an axis of uncertainty about female natural mortality for the base model. Columns range over low, mid, and high states of nature, and rows range over different assumptions of catch levels. The ABC catch streams are based on the equilibrium yield applying the SPR30 harvest rate.', sep = '')), 
